	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('freakyStart'), 0);
		...
		var bgEyes:FlxSprite = new FlxSprite();
		bgEyes.frames = Paths.getSparrowAtlas('titleEyes');
		bgEyes.animation.addByPrefix('idle', 'eyes animation', 24, true);
		bgEyes.animation.play('idle');
		bgEyes.antialiasing = true;
		bgEyes.updateHitbox();
		add(bgEyes);

		var glowEyes:FlxSprite = new FlxSprite();
		glowEyes.frames = Paths.getSparrowAtlas('eyesGlow');
		glowEyes.animation.addByPrefix('idle', 'eyes glow animation', 24, true);
		glowEyes.animation.play('idle');
		glowEyes.antialiasing = true;
		glowEyes.updateHitbox();
		add(glowEyes);
		...
		titleText = new FlxSprite(330, FlxG.height * 0.8);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// Friday
				case 13:
					addMoreText('Reimagined Corruption');
				// Night'
				case 14:
					deleteCoolText(); // at bubuski smells
				case 15:
