 ...
 */
class AlphabetBside extends FlxSpriteGroup
{
	...
	var lastSprite:AlphaCharacterBside;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	var isBold:Bool = false;
	public var lettersArray:Array<AlphaCharacterBside> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = AlphaCharacterBside.numbers.indexOf(character) != -1;
			var isSymbol:Bool = AlphaCharacterBside.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = AlphaCharacterBside.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacterBside = new AlphaCharacterBside(30 * loopNum, 0, textSize);
				var letter:AlphaCharacterBside = new AlphaCharacterBside(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = AlphaCharacterBside.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = AlphaCharacterBside.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = AlphaCharacterBside.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacterBside = new AlphaCharacterBside(30 * loopNum, 0, textSize);
				var letter:AlphaCharacterBside = new AlphaCharacterBside(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class AlphaCharacterBside extends FlxSprite
{
	...
	public function new(x:Float, y:Float, textSize:Float, isBside:Bool = false)
	{
		super(x, y);

		frames = Paths.getSparrowAtlas('alphabet_bside');	
