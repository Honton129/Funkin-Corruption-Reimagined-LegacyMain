...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var overlay:BGSprite = new BGSprite('stageoverlay', 0, 0, 0.9, 0.9);
		overlay.cameras = [camOverlay];
		overlay.antialiasing = ClientPrefs.globalAntialiasing;
		add(overlay);

		// Characters
		gf = new Character(400, 130, 'gf-bed');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		boyfriend = new Character(770, 100, 'bf-dad', true);
		boyfriend.x += boyfriend.positionArray[0];
		boyfriend.y += boyfriend.positionArray[1];
		dad = new Character(100, 100, 'dad-bed');
